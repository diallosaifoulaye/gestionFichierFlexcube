...
class BaseViews
{
     ...
     * @param array $config
     */
    public function __construct($lang, $url, $template, $config)
    {
...
//        $this->captcha = $this->captcha->displayHTML();
    ...
    public function initTemplate($param)
    {
        ...
        $this->sidebar =
            (isset($sidebar) && file_exists(ROOT . Prefix_View . 'template/'.$sidebar.'.php')) ? $sidebar : ((isset($header) && strtolower($header) === 'not-used') ? null : $this->sidebar);
        $this->footer  =
            (isset($footer) && file_exists(ROOT . Prefix_View . 'template/'.$footer.'.php')) ? $footer : ((isset($header) && strtolower($header) === 'not-used') ? null : $this->footer);
    }
